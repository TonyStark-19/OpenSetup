            ...
            {viewMode === "preview" ? (
                <div className="prose dark:prose-invert max-w-none text-left select-text">
                    <ReactMarkdown
                        components={{
                            h1: ({ children }) =>
                                <h1
                                    className="text-3xl font-extrabold tracking-tight text-zinc-900 dark:text-white mb-4 border-b 
                            ...
                            h2: ({ children }) =>
                                <h2
                                    className="text-xl font-bold tracking-tight text-zinc-800 dark:text-[#EDEEF0] mt-8 mb-3 flex items-center 
                            ...
                            ol: ({ children }) =>
                                <ol className="list-decimal list-inside text-zinc-600 dark:text-[#888a8e] text-[14px] space-y-2 mb-4 pl-1"
                                >
                                    {children}
                                </ol>,
                            li: ({ children }) =>
                                <li
                                    className="text-zinc-600 dark:text-[#888a8e] marker:text-zinc-400 dark:marker:text-[#525256]"
                                >
                            ...
                            code({ className, children, ...props }) {
                                ...
                                return className ? (
                                    <div className="my-5 space-y-2 w-full font-mono">
                                        ...
                                        >
                                            <div
                                                ...
                                                px-4 py-2 flex items-center justify-between text-zinc-400 dark:text-[#525256] text-xs 
                                    ...
                                    <code
                                        ...
                                        dark:text-[#EDEEF0] text-xs px-1.5 py-0.5 rounded font-mono transition-colors" {...props}
                                    >
                                        {children}
                                    </code>
                                );
                ...
                <div className="space-y-4 text-left select-text">
                    <div className="flex items-center gap-2 text-xs text-zinc-400 dark:text-[#888a8e] mb-1 font-mono select-none">
                        <FileText size={14} />
                        <span>{MOCK_METADATA.slug} (Raw Content Vector)</span>
                    </div>

                    <pre
                        className="bg-zinc-50 dark:bg-[#070708] border border-zinc-200 dark:border-[#1c1c1f] rounded-xl p-5 font-mono 
                        text-[13px] text-zinc-700 dark:text-[#b5b7ba] whitespace-pre-wrap break-all leading-relaxed m-0 overflow-x-auto 
