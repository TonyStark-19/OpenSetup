...
// base url
const BASE_URL = "http://localhost:5000/api/guides/helper";
    ...
    useEffect(() => {
        const fetchTelemetryData = async () => {
            try {
                ...
                const titleRes = await fetch(`${BASE_URL}/newest-title`);
        ...
        <div className="relative z-20 flex flex-col justify-center items-center w-full max-w-3xl mx-auto text-center pt-28">
            ...
            <Link to="/guides" className="block">
                <div
                    ...
                    dark:hover:border-[#38383c] transition-all duration-200 backdrop-blur-sm mb-8 animate-fade-in"
                >
                    {/* pulse animation dot */}
                    <div className="w-2 h-2 bg-emerald-500 dark:bg-[#4ade80] rounded-full mr-2.5 animate-pulse"></div>

                    {/* new guide text */}
                    <p className="text-zinc-500 dark:text-[#888a8e] text-[13px] font-medium line-clamp-1">
                        {newestTitle} just added
                    </p>

                    {/* link button */}
                    <p className="text-zinc-900 dark:text-[#EDEEF0] flex flex-row justify-center items-center text-[13px] font-medium ml-3 gap-0.5 shrink-0">
                        Read it
                        <ArrowRight
                            size={14}
            ...
            {/* Bottom Stats Meta */}
            <p className="text-zinc-400 dark:text-[#525256] text-xs md:text-sm mt-8 tracking-wide font-medium transition-colors duration-300 select-none">
                OPEN SOURCE ·{" "}
